...
char *expr_parenthesize(char *buf, const str name, int n, ...)
{
    va_list exprs;
    va_start(exprs, n);
    // TODO pre-reserve space for buf?
    arr_push(buf, '(');
...
char *expr_sprint(char *buf, const Expr *e)
{
    switch (e->type) {
        ...
        case EXPR_BINARY: return expr_parenthesize(buf, e->binary.op->name, 2, e->binary.lhs, e->binary.rhs);
        case EXPR_UNARY: return expr_parenthesize(buf, e->binary.op->name, 1, e->binary.rhs);
